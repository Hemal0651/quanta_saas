...
void main() {
  vec2 pad = vec2(0.008, 0.012);
  vec2 imgUv = (vUv - pad) / (vec2(1.0) - pad * 2.0);
  vec3 base;
  if (uHasImage > 0.5 && imgUv.x >= 0.0 && imgUv.x <= 1.0 && imgUv.y >= 0.0 && imgUv.y <= 1.0) {
    base = texture2D(tMap, imgUv).rgb;
  } else {
  ...
  // Top-only corner rounding SDF (q.y > 0 is bottom half, q.y <= 0 is top half)
  vec2 p = (vUv - 0.5) * uRes;
  ...
  q.x = abs(q.x);
  if (q.y > 0.0) {
    sd = max(q.x - halfRes.x, q.y - halfRes.y);
  } else {
    vec2 d = vec2(q.x, -q.y) - (halfRes - vec2(r, r));
    sd = length(max(d, 0.0)) + min(max(d.x, d.y), 0.0) - r;
  }
